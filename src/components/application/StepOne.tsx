...
export const StepOne = ({ teamMembers, onTeamMembersChange }: StepOneProps) => {
  ...
  const searchDB = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Append name to list
        const updatedMembers = [...teamMembers, data.name];
          ...
          <div className="ml-8 mt-2 w-2/3">
            ...
            <div className="flex flex-col gap-2">
              {teamMembers.map((name, index) => (
                <div
                  key={index}
                  className="flex justify-between items-center bg-gray-200 text-gray-800 px-3 py-1 rounded shadow-sm w-full"
                >
                  <span>{name}</span>
                  <button
                    type="button"
